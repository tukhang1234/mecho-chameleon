...
// Chú ý: Đường dẫn đã được sửa thành thư mục "pictuer" của bạn
// ==========================================
const titanAssets = {
    body: new Image(),
    gunArm: new Image(),
...
class Player {
    ...
    updateTitanLogic(mouse) {
        ...
        if (this.titanAttackCd <= 0 && typeof enemies !== 'undefined') {
            ...
            if (target || mouse.down) {
                ...
                if (Math.random() > 0.5) {
                    // Bắn súng - Kích hoạt giật lùi (Railgun)
    ...
    // ==========================================
    // VẼ CỖ MÁY TITAN (ASSEMBLY CORRECTED)
    // ==========================================
    _drawTitan(ctx) {
        ctx.save();
        ...
        // --- VẼ SÚNG ---
        ctx.save();
        ctx.translate(54, 39 + breath + this.railgunRecoil);
        ctx.rotate(0 * Math.PI / 180);
        ctx.drawImage(titanAssets.gunArm, -26, -43, 52, 85);
        ctx.restore();

        // --- VẼ BÚA ---
        ctx.save();
        ctx.translate(-53, 41 + breath);
        ctx.rotate(0 * Math.PI / 180);
        ...
        ctx.drawImage(titanAssets.body, -80, -85, 160, 160);
